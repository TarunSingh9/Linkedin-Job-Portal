...
const MyJobs = () => {
    ...
    useEffect(() => {
        setIsLoading(true);
        fetch(`http://localhost:3000/myJobs/tarunreal33425@gmail.com`)
        .then(res => res.json())
        .then(data => {
        setJobs(data);
        setIsLoading(false);
  ...
  return  <div className="max-w-screen-2xl container mx-auto x1:px-24 px-4">
    <div className="my-jobs-container">
       <h1 className="text-center p-4">All My Jobs</h1>
      <div className='search-box p-2 text-center mb-2'>
          <input 
           onChange={(e) => setSearchText(e.target.value)} 
          type='text' name='search' id='search' className='py-2 p1-3 border focus:outline-none lg:w-6/12 mb-4 w-full'/>
    ...
    <div className="rounded-t mb-0 px-4 py-3 border-0">
      <div className="flex flex-wrap items-center">
        ...
        <div className="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
          <Link to="/post-jobs"><button className="bg-indigo-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">Post a New Job</button></Link>
        </div>
      </div>
    </div>
